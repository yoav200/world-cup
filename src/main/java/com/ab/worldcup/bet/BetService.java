...
public class BetService {
  ...
  public void setQualifiersBets(Account account, Map<Stage, List<Qualifier>> userQualifiersByStageMap) {
    ...
    Map<Long, UserBet> userBetMap = betForAccount.stream()
  ...
  public BetStatisticsData getBetStats(Long betId) {
    ...
    if (betsCount > 0) {
      homeTeamBets =
          (int) allUserBetsByBetId.stream().filter(b -> b.getHomeTeamGoals() > b.getAwayTeamGoals()).count() * 100
              / betsCount;
      awayTeamBets =
          (int) allUserBetsByBetId.stream().filter(b -> b.getHomeTeamGoals() < b.getAwayTeamGoals()).count() * 100
              / betsCount;
      drawBets =
          (int) allUserBetsByBetId.stream().filter(b -> b.getHomeTeamGoals().equals(b.getAwayTeamGoals())).count() * 100
              / betsCount;
    }
