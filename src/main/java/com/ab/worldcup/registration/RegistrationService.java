...
public class RegistrationService {
  ...
  public Account register(RegistrationRequest request) {
    ...
    return sendValidationEmail(newAccount.getEmail());
  }
  ...
  Account sendValidationEmail(String email) {
  ...
  Account changePassword(RegistrationRequest request) {
    ConfirmationToken confirmationToken =
        confirmationTokenService
    ...
    if (!confirmationToken.getAccount().getEmail().equals(request.getEmail())) {
      throw new BadRequestException("Token not match email");
    }
  ...
  private void validateEmail(String email) {
    ...
    if (CollectionUtils.isNotEmpty(applicationConfig.getAllowedDomains())) {
      String allowedDomains = String.join(", ", applicationConfig.getAllowedDomains());
      boolean match = applicationConfig.getAllowedDomains().stream().anyMatch(email::endsWith);
      if (!match) {
        throw new BadRequestException("Invalid email, not allowed domain [" + allowedDomains + "]");
      }
    }
  }
