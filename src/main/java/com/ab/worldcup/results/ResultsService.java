...
public class ResultsService {
  ...
  public RankingData getRankingForAccount(Account account, List<UserBet> betForAccount, int qualifierBetsCount,
      int groupMatchesCount, int knockoutMatchesCount) {
    List<CalculatedUserBet> calculatedUserBets = calculateBetsForUser(betForAccount);
    int qualifierBetProgressPercentage =
        (int) calculatedUserBets.stream().filter(t -> t.getBetType().equals(BetType.QUALIFIER)).count() * 100
            / qualifierBetsCount;
    int groupMatchesBetProgressPercentage = (int) calculatedUserBets.stream().filter(
        t -> t.getBetType().equals(BetType.MATCH) && t.getUserBet().getUserBetId().getBet().getStageId()
            .equals(Stage.GROUP)).count() * 100 / groupMatchesCount;
    int knockoutMatchesBetProgressPercentage = (int) calculatedUserBets.stream().filter(
        t -> t.getBetType().equals(BetType.MATCH) && !t.getUserBet().getUserBetId().getBet().getStageId()
            .equals(Stage.GROUP)).count() * 100 / knockoutMatchesCount;
  ...
  public List<CalculatedUserBet> calculateBetsForUser(List<UserBet> betForAccount) {
    ...
    Map<Long, MatchResult> resultMap = getAllMatchResults().stream()
  ...
  public MatchResult findMatchResultByMatchId(Long matchId) {
    return matchResultRepository.findById(matchId).orElseThrow();
  }
}
